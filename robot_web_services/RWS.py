...
class RWS:
    ...
    def set_rapid_variable_L(self, var, value):
        """Sets the value of any RAPID variable.
        Unless the variable is of type 'num', 'value' has to be a string.
        """

        payload = {'value': value}
        resp = self.session.post(self.base_url + '/rw/rapid/symbol/data/RAPID/T_ROB_L/' + var + '?action=set',
                                 data=payload)
        return resp
    
    def set_rapid_variable_R(self, var, value):
        """Sets the value of any RAPID variable.
        Unless the variable is of type 'num', 'value' has to be a string.
        """

        payload = {'value': value}
        resp = self.session.post(self.base_url + '/rw/rapid/symbol/data/RAPID/T_ROB_R/' + var + '?action=set',
                                 data=payload)
        return resp

    def get_rapid_variable_L(self, var):
        """Gets the raw value of any RAPID variable.
        """

        resp = self.session.get(self.base_url + '/rw/rapid/symbol/data/RAPID/T_ROB_L/' + var + ';value?json=1')
        json_string = resp.text
        _dict = json.loads(json_string)
        value = _dict["_embedded"]["_state"][0]["value"]
        return value
    
    def get_rapid_variable_R(self, var):
        """Gets the raw value of any RAPID variable.
        """

        resp = self.session.get(self.base_url + '/rw/rapid/symbol/data/RAPID/T_ROB_R/' + var + ';value?json=1')
        json_string = resp.text
        _dict = json.loads(json_string)
        value = _dict["_embedded"]["_state"][0]["value"]
        return value

    def get_robtarget_variables_L(self, var):
    ...
    def get_robtarget_variables_R(self, var):
        """Gets both translational and rotational data from robtarget.
        """
    ...
    #!!!!!!!!!!!!!!!!!! get_gripper_position MUSS NOCH ABGEÄNDERT WERDEN, FUNKTIONIERT AKTUELL NICHT !!!!!!!!!!!!!!
    ...
    #!!!!!!!!!!!!!!!!!! get_gripper_height MUSS NOCH ABGEÄNDERT WERDEN, FUNKTIONIERT AKTUELL NICHT !!!!!!!!!!!!!!
        ...
        if rot == [0, 0, 0, 0]:  # If the target has no previously defined orientation
            ...
            self.set_rapid_variable_L(var, "[[" + ','.join(
                [str(s) for s in trans]) + "],[" + ','.join(str(s) for s in rot) + "],[-1,0,0,0],[9E+9,9E+9,9E+9,9E+9,"
                                                                                   "9E+9,9E+9]]")
    def set_robtarget_translation_R(self, var, trans):
        """Sets the translational data of a robtarget variable in RAPID.
        """

        _trans, rot = self.get_robtarget_variables_R(var)
        if rot == [0, 0, 0, 0]:  # If the target has no previously defined orientation
            self.set_rapid_variable_R(var, "[[" + ','.join(
                [str(s) for s in trans]) + "],[0,1,0,0],[-1,0,0,0],[9E+9,9E+9,9E+9,9E+9,9E+9,9E+9]]")
        else:
            self.set_rapid_variable_R(var, "[[" + ','.join(
                [str(s) for s in trans]) + "],[" + ','.join(str(s) for s in rot) + "],[-1,0,0,0],[9E+9,9E+9,9E+9,9E+9,"
    ...
    def set_robtarget_rotation_z_degrees_L(self, var, rotation_z_degrees):
        ...
        trans, _rot = self.get_robtarget_variables_L(var)

        self.set_rapid_variable_L(var, "[[" + ','.join(
            [str(s) for s in trans]) + "],[" + ','.join(str(s) for s in rot) + "],[-1,0,0,0],[9E+9,9E+9,9E+9,9E+9,"
                                                                               "9E+9,9E+9]]")
        
    def set_robtarget_rotation_z_degrees_R(self, var, rotation_z_degrees):
        """Updates the orientation of a robtarget variable
        in RAPID by rotation about the z-axis in degrees.
        """
        ...
        trans, _rot = self.get_robtarget_variables_R(var)
    ...
    def set_robtarget_rotation_quaternion_L(self, var, rotation_quaternion):
        ...
        self.set_rapid_variable_L(var, "[[" + ','.join(
            [str(s) for s in trans]) + "],[" + ','.join(str(s) for s in rotation_quaternion) + "],[-1,0,0,0],[9E+9,"
                                                                                               "9E+9,9E+9,9E+9,9E+9,"
                                                                                               "9E+9]]")
    def set_robtarget_rotation_quaternion_R(self, var, rotation_quaternion):
        """Updates the orientation of a robtarget variable in RAPID by a Quaternion.
        """
